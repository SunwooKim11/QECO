    ...
    def _build_net(self):
        class Net(nn.Module):
            def __init__(self, dueling, hidden_units_l1, n_lstm, n_features, n_actions, n_lstm_state, num_layers):
                ...
                self.lstm = nn.LSTM(input_size=n_lstm_state, num_layers=num_layers, hidden_size=n_lstm, batch_first=True, bidirectional=True)
            ...
            def forward(self, s, lstm_s):
                h_0 = Variable(torch.zeros(self.num_layers * 2, lstm_s.size(0), self.hidden_size)).to(self.device)
                c_0 = Variable(torch.zeros(self.num_layers * 2, lstm_s.size(0), self.hidden_size)).to(self.device)
                lstm_out, _ = self.lstm(lstm_s, (h_0, c_0))
                lstm_out = lstm_out[:, -1, :]
