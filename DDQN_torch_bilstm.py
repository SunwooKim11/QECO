...
class DuelingDoubleDeepQNetwork(nn.Module):
    ...
    def _build_net(self):
        class Net(nn.Module):
            def __init__(self, dueling, hidden_units_l1, n_lstm, n_features, n_actions, n_lstm_state, num_layers):
                super(Net, self).__init__()
                self.dueling = dueling

                self.lstm = nn.LSTM(input_size=n_lstm_state, hidden_size=n_lstm//2, batch_first=True, bidirectional=True)
            ...
            def forward(self, s, lstm_s):
                # h_0 = Variable(torch.zeros(2, lstm_s.size(0), self.N_lstm)).to(self.device)
                # c_0 = Variable(torch.zeros(2, lstm_s.size(0), self.N_lstm)).to(self.device)
        ...
        return Net(self.dueling, self.hidden_units_l1, self.N_lstm, self.n_features, self.n_actions, self.n_lstm_state, self.num_layers)
    ...
    def learn(self):
        ...
        with autocast():
            ...
            reward = batch_memory[:, self.n_features + 1].half()
            ...
            selected_q_next = selected_q_next.half()
            gamma = torch.tensor(self.gamma, dtype=torch.float16).to(self.device)  # gamma를 half로 변환

            # print("q_eval, q_target, reward, q_next", q_eval.dtype, q_target.dtype, reward.dtype, selected_q_next.dtype)
